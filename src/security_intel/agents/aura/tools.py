"""Aura agent tools (EASM capability) — loaded from the configured remote MCP server.

Tools come from the real EASM MCP server (configured via EASM_MCP_URL or
MCP_SERVERS). When no server is configured, no tools are returned and the Aura
